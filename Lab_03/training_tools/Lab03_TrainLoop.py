...
BEST_LOG_PATH = "best_log.csv"
BEST_TERMINAL_OUTPUTS_FILE = "best_terminal_outputs_log.txt"
...
def main_train(seed: int):
    ...
    return score_final_gram1 / len(translation_data)
...
if __name__ == "__main__":
    ...
    for i in range(MAX_LOOP):
        ...
        # Run training loop
        test_bleu = main_train(seed)

        # Copy file to best
        if test_bleu > best_test_bleu:
            best_test_bleu = test_bleu
            os.replace(LOG_TERMINAL_OUTPUTS_FILE, BEST_TERMINAL_OUTPUTS_FILE)
            os.replace(LOG_FILE_PATH, BEST_LOG_PATH)
            os.replace(MODEL_SAVE_PATH, BEST_MODEL_PATH)
            print(f"New best test BLEU: {best_test_bleu:.4f}\n")
